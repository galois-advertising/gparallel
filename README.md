...
`meta`对`meta_storage_t`子集的一个指定，通过定义`getter`和`setter`来说明自己中包含的数据元素。子集之间可以互相包含，原理与面向对象中的`继承`是一样的。同理，如果一个任务依赖于一个`meta`，则也同样依赖于这个`meta`的父`meta`。`继承`机制的主要目的是为了避免重复定义集合的元素，增加代码的可维护性。
